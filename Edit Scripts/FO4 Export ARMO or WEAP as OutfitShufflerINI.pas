...
begin
    ...
    if (rec <> 'ARMO')  and (rec <> 'WEAP') then begin
    ...
    // Common elements
	if FixedFormID(e)<16777216 then begin
		formid    := IntToHex(FixedFormID(e),6);
		tempint := FixedFormID(e);
	end;
	if FixedFormID(e)>16777216 then begin
	...
	row := Trim(row);
    row := row + ' ;' + formid + ';' + edid  + ';'  + EditorID(e) + ';' +  bodyslots;
	row := Trim(row);
    slRows.Add(row);
	bodySlotList.Free;
end;
...
begin
	...
	if (slRows.Count > 0) then begin
		...
		dlgSave.InitialDir := ProgramPath+'Data\F4SE\OutfitShuffler\';
